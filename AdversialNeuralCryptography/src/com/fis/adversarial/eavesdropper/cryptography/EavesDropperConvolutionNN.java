package com.fis.adversarial.eavesdropper.cryptography;

import static com.fis.adversarial.cryptography.GlobalConstants.EAVESDROPPER;
...
import static com.fis.adversarial.cryptography.GlobalConstants.nEavesdropperNNOutcome;
...
public class EavesDropperConvolutionNN {

	private static final Random rng = new Random(seed);
	private MultiLayerNetwork model;

	//private default no-arg constructor
	private EavesDropperConvolutionNN() {

	}

	//factory method to create instance of this class
	public static EavesDropperConvolutionNN getInstance() {
		return new EavesDropperConvolutionNN();
	}
	...
	private MultiLayerNetwork constructConvolutionalNeuralNetwork() {
		 ...
		 */
		OutputLayer outputLayer = helper.fullyConnectedOutputLayer(FC_OUTPUT_LAYER, nEavesdropperNNOutcome);
	...
	public void doTrainModel() throws Exception {
		DataSetIterator dataSetItr = AdversarialNeuralHelper.getInstance().loadTrainingData(EAVESDROPPER);
	...
	public void saveTrainedConvolutionalModel(MultiLayerNetwork model) throws Exception {
		AdversarialNeuralHelper.getInstance().saveTrainedModel(model,EAVESDROPPER);
	}

	public INDArray evaluateResult(INDArray input) throws Exception {
		if (this.model == null) {
			this.model = AdversarialNeuralHelper.getInstance().loadTrainedModel(EAVESDROPPER);
		}
	...
	public static void main(String[] args) throws Exception {
		EavesDropperConvolutionNN convolutionNN = new EavesDropperConvolutionNN();
